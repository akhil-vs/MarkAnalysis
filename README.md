...
Deployments run `prisma migrate deploy` during the Vercel build so `Period` / `TimetableEntry` tables stay in sync. If periods are empty after migrate, the API seeds a default bell schedule automatically.
