# Teacher user manual

School Marks Analytics — guide for the **Teacher** role.

Teachers enter and submit marks for assigned papers, respond to leadership notices, review their own class analytics, and (if they are a **class teacher**) download consolidated lists for their section when every paper is approved.

---

## 1. Getting access and signing in

### 1.1 Request access

1. Ask your principal for the school’s **join code** (shown on the school’s School profile).
2. Open `/signup`, enter the join code, and complete your details (name, email, school ID, password, role).
3. Wait until leadership **activates** your account and assigns your **class + subject** papers. You cannot enter marks until both are done.

Alternatively, leadership may create your account and share credentials. If they reset your password, you may be asked to change it on first login.

### 1.2 Sign in

1. Open the login page.
2. Enter **email**, **school ID**, and **password**.
3. If MFA is on, enter the authenticator code.

You land on the **Teacher desk**: your papers for the current exam, register status, notices, section strength, and students to watch.

---

## 2. Teacher desk and notices

- Switch exams with the **exam selector** (you share the same latest exam as leadership by default).
- **Notices** appear on the desk and in the **bell** (deadlines, incomplete marklists, late-entry/edit decisions, staff messages). Open a notice to jump into the relevant register.
- Metrics show your average, student count, sections, and how many registers still need finishing.
- Class teachers see their section(s) called out when relevant.

---

## 3. Entering marks

### 3.1 Mark register (`Marks → Mark register`)

1. Choose the **exam**, **class/section**, and **subject** you teach.
2. Enter numeric marks within the paper’s max marks.
3. Use special codes where needed:

| Code | Meaning |
|---|---|
| `AB` | Absent |
| `EX` | Exempt |
| `WH` | Withheld |

4. **Save draft** as you work. Drafts are not visible in school analytics or consolidated lists.
5. When the paper is complete, **Submit** for principal / co-ordinator approval.

Only **approved** marks feed ranks, school reports, and official consolidated lists. After submit, you generally cannot edit without an approved **access request**.

### 3.2 Bulk upload (`Marks → Bulk upload`)

1. Download the template for your class and exam.
2. Fill the spreadsheet, upload, and review the **preview**.
3. Commit only when the preview matches the roll (missing students and bad values are flagged first).

### 3.3 Class teacher read access

If you are the **class teacher** of a section, you can open that section’s full register in a **read-only** view for papers you do not teach — useful to see which subject registers are still empty or awaiting approval.

### 3.4 Late entry and edit requests

If the deadline has passed, or you need to change a submitted register:

1. Request **late entry** or **edit** access (from the register flow / access request path leadership uses).
2. Wait for principal or co-ordinator approval (you will get a notice).
3. Complete or correct the register, then submit again if required.

Do not ask for late access unless a genuine correction is needed — grants are audited.

---

## 4. Consolidated lists (class teachers)

`Marks → Consolidated lists` appears when you are class teacher of at least one section.

1. Choose exam → your class → division.
2. You only see the list when **every subject register** for that section is fully **approved**. Incomplete divisions are not shown to class teachers.
3. Download Excel or PDF for your section’s official list once it is ready.

Leadership can preview incomplete lists; class teachers cannot.

---

## 5. Hall tickets

`Marks → Hall tickets` — teachers can **view and download** tickets for their classes. Principals and co-ordinators create and edit batch details (venue, instructions, schedule). If a PDF will not download, paper date/time may still be missing under Records → Exams.

---

## 6. Insights (your lens)

Under **Insights → Marks analysis** teachers typically use:

| Report | What you get |
|---|---|
| **Classes** | Your sections: subject stats and rankings |
| **Students** | Search a student you teach; trends, strengths/weaknesses, report card |

School-wide subject/teacher leaderboards, Compare, and Deep insights are leadership tools. Your desk already summarises your papers and students to watch.

---

## 7. CPD

If Continuing Professional Development is enabled for the school, open **CPD** to see **your** training plans, observations, appraisals, and certificates. Leadership records observations and appraisals against your file.

---

## 8. Profile and security

**Account → Profile**:

- Keep name and email current.
- Change your password.
- Enable **MFA** (authenticator app) and store recovery codes safely.

---

## 9. Typical teacher checklist (per exam)

1. Confirm your assigned papers on the Teacher desk.
2. Enter or bulk-upload marks; save drafts as you go.
3. Double-check absences (`AB` / `EX` / `WH`) before submit.
4. **Submit** each complete register before the deadline.
5. Watch the bell for reminders or rejection/approval of access requests.
6. If you are class teacher, check when the section becomes ready for the **consolidated list** and download it.
7. Download **hall tickets** for your classes when leadership has prepared them.
8. Use **Students** analysis for parent meetings or remedial follow-up.

---

## 10. What teachers cannot do

Teachers do not:

- Approve or moderate other teachers’ marks
- Manage staff, school profile, or whole-school records
- See incomplete consolidated lists
- Open leadership-only analysis (school overview, subjects hub, teacher comparison, deep insights) unless their role features are customised by the principal

If a paper is missing from your register list, ask leadership to check your **assignments** under Staff.
