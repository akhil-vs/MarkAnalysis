...
describe("ensureSchema bootstrap", () => {
  ...
  it("embeds activity audit statements matching the prisma migration checksum", () => {
    ...
    assert.ok(__test.ACTIVITY_ACTIONS.includes("USER_ASSIGNMENTS_TRANSFERRED"));
  });
  ...
  it("lists every catch-up migration for the cold-start fast path", () => {
    ...
    assert.ok(__test.CATCHUP_MIGRATION_NAMES.includes(__test.MFA_USER_MIGRATION));
    assert.ok(__test.CATCHUP_MIGRATION_NAMES.includes(__test.LIVE_OPS_MIGRATION));
    assert.ok(__test.AUTH_CATCHUP_MIGRATION_NAMES.includes(__test.MFA_USER_MIGRATION));
    assert.ok(
