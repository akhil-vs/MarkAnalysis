...
export default function ConsolidatedLists() {
  ...
  const [maxMarksLocked, setMaxMarksLocked] = useState(null);
  ...
  useEffect(() => {
    ...
    if (leadership) {
      api("/api/consolidation/max-marks")
        .then((res) => setMaxMarksLocked(Boolean(res.settings?.maxMarksLocked)))
        .catch(() => setMaxMarksLocked(null));
    }
  }, []);
    ...
    return (
      <div>
        <PageHeader
          ...
          subtitle={
            leadership
              ? "After teachers enter and you approve marks, generate the official class list."
        ...
        <div className="card p-4 mb-5 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-ink-700/70 mt-0.5">
              {maxMarksLocked
                ? "Subject ceilings are locked. Totals and percentages use Records → Subjects max marks."
          ...
          <Link className="btn-ghost" to="/manage?tab=Subjects">
            {maxMarksLocked ? "View subjects" : "Set max marks"}
          </Link>
        </div>
      )}
