  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={`${data.exam.name} — teachers who have not finished their assigned registers`}
        actions={
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 mb-6">
        ...
        <Kpi label="Teachers complete" value={data.completeTeacherCount} />
          ...
          {pending.map((t) => (
            <div key={t.teacherId} className="card p-4">
              <div className="flex flex-wrap items-baseline justify-between gap-2">
                ...
                <div className="text-sm text-clay-600">{t.missingAssignments} register{t.missingAssignments === 1 ? "" : "s"} outstanding</div>
              </div>
              <PaginatedTable
                items={t.assignments.filter((a) => a.missing > 0)}
                pageSize={5}
                pageSizeOptions={[5, 10, 25]}
                empty="No missing registers."
              >
                {(page) => (
